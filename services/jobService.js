...
const AppliedInfo = require("../models/AppliedInfoModel");
...
exports.findAllJobService = async (query) => {
    const jobs = await Job.find({ ...query?.data })
        .select({ ...query?.select })
        .sort(query?.sort)
        .limit(query?.limit)
...
exports.findOneJobService = async (query) => {
    const job = await Job.findOne({ ...query?.data })
        .select({ ...query.select })
        .populate(query.populate);
    return job;
};
...
exports.appliedJobInfoService = async (data) => {
    ...
    const updateJob = await Job.updateOne(
...

        {
            $inc: { totalApplied: 1 },
            $push: {
                appliedInfo: { id: appliedInfoId, candidateId: candidate },
            },
    ...
    const updateCandidate = await Candidate.updateOne(
        { _id: candidate },
        { $push: { appliedInfo: appliedInfoId, jobs: job } }
    );
    return applyJob;
};
