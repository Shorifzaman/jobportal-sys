...
exports.createJob = async (req, res, next) => {
    try {
        const deadline = new Date(
            new Date().setDate(
                (new Date().getDate() + Math.round(Math.random() * 10)) % 30
            )
        );
...
exports.applyJob = async (req, res, next) => {
    try {
        const { _id } = req.user;
        const currentCandidate = await candidateService.findOneCandidateService(
            { data: { "user.id": _id } }
        );
        ...
        if (!jobData) {
            return res
                .status(400)
                .json({ success: false, error: "No Job Found" });
        }
        ...
        const isApplied = await JobService.findAppliedJobService(
            appliedJobQuery
        );
